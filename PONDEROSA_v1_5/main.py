...
'''def get_training(self,df,lab_type,val_list):
			...
			def get_putative(self,df,val_list):
				vals,ids = df[val_list].values.tolist(),df["PAIR_ID"].values.tolist()
				return vals,labs

			def find_putative(self):
				train_val,train_lab = self.get_training(self.training,"DEGREE",["IBD1","IBD2"])
				classif = LinearDiscriminantAnalysis().fit(train_val,train_lab)
				self.putative["SECOND_PROB"] = self.putative.apply(lambda x: classif.predict_proba([x.IBD1,x.IBD2])[0][2],axis=1)
				self.putative = self.putative["SECOND_PROB"] > threshold

			def classify_second(self):
				train_val,train_lab = self.get_training(self.training[self.training["REL"]=="2nd"],"REL",["HSR","N"])
				classif = LinearDiscriminantAnalysis().fit(train_val,train_lab)
				probs = clasif.predict_proba(self.putative[["HSR","N"]].values.tolist())
				for index,rel in enumerate(self.second):
					self.putative[rel] = [p[i] for p in probs]
			...
			def ages(self,df): #constrain rels with age info
				df["CONDITION"] = abs(df["AGE1"]-df["AGE2"]) > mhs_gap
				self.set_zero("MHS",df)
				df["CONDITION"] = abs(df["AGE1"]-df["AGE2"]) < gp_gap
				self.set_zero("GP",df)
			...
			def write_out(out,df):
				self.set_conditional(df)
				df["REL"] = df[second].idxmax(axis=1)
				df["PROB"] = df[second].max(axis=1)
				df["AV_ERROR"] = np.where(df["REL"] == "AV",df.apply(lambda x: self.av_error(x.H1,x.AGE1,x.H2,x.AGE2),axis=1),False)
				with open("%s_second.txt" % out,"w") as outfile:
					outfile.write(df.to_string(index=False,na_rep="NA",columns=["PAIR_ID","YOUNGER","OLDER","REL","PROB","HSR","N","AV","GP","MHS","PHS"])


			def run(self):
				self.find_putative()
				self.classify_second()
				self.putative = resolve_generations(self.putative,agef,"YOUNGER","OLDER")
				self.ages(self.putative)
				self.parent(self.putative)
				self.write_out(out,self.putative)
...
#First step is to calculate hap scores
	def run_hapscores(kingf):
		relative_list = [[lines.split()[1],lines.split()[3]] for lines in open(kingf).readlines()[1:]]
		relative_list = [min(pair) + "_" + max(pair) for pair in relative_list]
		return hap.get_hap_score(relative_list,matchf,mapf,pedf,out,num_chr)

	#Input if a df with IID1,IID2 and hap scores calculated; output is the same df with 4 new fields:
	#AGE1,AGE2,younger ind,older ind. If age data is available for both, younger/older is determined by age.
	#Else: determined by individual haplotype scores
	def resolve_generations(df,agef,young_lab,old_lab):
		def add_age(df):
			age_data = {}
			if agef != None:
				age_data = {lines.split()[0]:int(lines.split()[1]) for lines in open(agef).readlines()}
		...
		add_age(df)
		w_age = resolve_age(df[df["USE_H"]==False].copy())
		wo_age = resolve_h(df[df["USE_H"]].copy())
		df = pd.concat(w_age,wo_age)
		return df

	#Takes all king PO pairs, runs them through the resolve_gen function to orient them
	def po_analysis(hap_df):
		po = [[lines.split()[1],lines.split()[3]] for lines in open(kingf).readlines()[1:] if lines.split()[-1] == "PO"]
		po_data = pd.DataFrame([min(pairs) + "_" + max(pairs) for pairs in po],columns=["PAIR_ID"])
		po_data = pd.merge(po_data,hap_df,on="PAIR_ID",how="left")
		po_data = resolve_generations(po_data,"CHILD","PARENT")
		with open("%s_PO.txt" % out,"w") as outfile:
			  outfile.write(po_data.to_string(index=False))

	#Classifies putative 2nd degree relatives. Calculates hap scores, trains classifier, runs putative through classifier,
	#orients GP/AV pairs based on age or h, resets probs to 0 if impossible (e.g. age constraints),writes out putative
	def infer_second(king_df,relative_df,hap_df):
		class Data:
			def __init__(self,king,rel,hap_scores):
				#remove outliers; main goal is to remove duplicate ind (who have >1 rel)
				degrees = ["PO","FS","2nd","3rd"]
				def remove_outliers(df):
					df = df[df["GTD"] & df["DEGREE"].isin(degrees)]
					...
					for degree in degrees:
						...
						sd = stat.stdev(df[df["DEGREE"]==degree]["IBD1"])
						avg_sd[degree] = [mean,sd]
					df = df[df.apply(lambda x: abs(x.IBD1-avg_sd[x.DEGREE][0])/avg_sd[x.DEGREE][1] < zscore)]
					df.drop_duplicates(subset=["PAIR_ID"])

				self.putative = king[king[0.30 < king["IBD1"] < 0.75]]
				self.putative = pd.merge(self.putative,hap_scores,on="PAIR_ID",how="left").dropna()
				remove_outliers(rel)
				self.putative = pd.merge(self.putative,rel,on="PAIR_ID",how="left")
	...
	#Step 2: compute HSR for all pairs IBD1 > 0.3 (PO,FS,2nd)
	hap_df = run_hapscores(kingf)

	#Step 3: analyze PO pairs in king
	po_analysis(hap_df)

	#If usr only wants PO analysis, exit
	if run_type == "po_only":
		sys.exit()

	#Step 4: create ped structure; resolve amb sibships, add missing parents, creates 2 df: king_df, rel_df,
	pedigree = graphs.Pedigree()
	pedigree.run_PONDEROSA(kingf,famf)
	relative_df = pedigree.get_rels()
	king_df = pedigree.get_king()

	#If usr only wants ped analysis, exit
	if run_type == "ped_only":
		sys.exit()

	#Step 5: 
	infer_second(king_df,relative_df,hap_df)'''
