...
parser.add_option("--second_train",dest="train_2nd",default="")
            ...
            if option.train_2nd == "":
                for labs,rels in enumerate(ped_rels["2nd"]):
                    for pairs in self.get_relatives(rels):
                        iid1,iid2 = pairs[0],pairs[1]
                        if not self.gt_status(iid1) or not self.gt_status(iid2):
                            continue
                ...
                for pairs in open(option.train_2nd).readlines()[1:]:
                    pairs = pairs.split()
                    hap_data = [float(pairs[4]),int(pairs[5])]
...
def main(fam_file,match_file,king_file,map_file,
    ...
    if option.consider_fs:
        for lines in open(king_file).readlines()[1:]:
            if "FS" in lines and float(lines.split()[7]) > 0.15:
                lines = lines.split()
                iid1,iid2 = lines[1],lines[3]
                data.add_siblings(iid1,iid2,1.0,"FS")
    ...
    for lab,deg in enumerate(degrees[:3]):
        for rels in ped_rels[deg]:
            for pairs in data.get_relatives(rels):
                iid1,iid2 = pairs[0],pairs[1]
                if not data.gt_status(iid1) or not data.gt_status(iid2):
                    continue
                ibd1,ibd2 = data.get_ibd(iid1,iid2,1),data.get_ibd(iid1,iid2,2)
